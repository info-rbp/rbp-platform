        ...
        <section className="border-b border-slate-200 bg-slate-50">
          <div className="mx-auto max-w-7xl px-4 py-4 text-sm text-slate-600 sm:px-6 lg:px-8">
            ...
            {!isLoading && errorMessage ? (
              <p>
                Live resource content is unavailable right now. Showing curated placeholders instead.
              </p>
            ) : null}
            {!isLoading && !errorMessage && showingFallbackResources ? (
              <p>Showing curated placeholder resources while the live library is being prepared.</p>
            ) : null}
          </div>
        </section>
      )}
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
            {categories.map((cat) => {
              ...
              return (
                ...
                >
                  <div
                    className={`mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-xl ${cat.color}`}
                  >
          ...
          {filteredResources.length === 0 ? (
            <div className="rounded-3xl border border-slate-200 bg-slate-50 px-6 py-12 text-center shadow-sm">
              ...
              <p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-slate-600">
                We’re preparing guides, templates, and support material for members and business owners.
              </p>
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {filteredResources.map((resource) => (
                <div
                  key={resource.id}
                  className="flex flex-col rounded-2xl border border-slate-100 bg-white p-7 shadow-sm transition-all hover:shadow-md"
                >
        ...
        <div className="mx-auto max-w-2xl px-4 text-center sm:px-6">
          ...
          <div className="mx-auto flex max-w-md flex-col gap-3 sm:flex-row">
            <input
              ...
              className="flex-grow rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm transition-all focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
            <button className="whitespace-nowrap rounded-xl bg-blue-700 px-6 py-3 font-bold text-white transition-all hover:bg-blue-800">
              Preview Subscribe
            </button>
          </div>
          <p className="mt-3 text-xs text-slate-400">
            Frontend-only placeholder. No email subscription or backend record is created.
          </p>
        </div>
      </section>
